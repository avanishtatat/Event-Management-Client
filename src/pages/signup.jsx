...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/auth/signup`, option)
  ...
  return (
    <div className="bg-gray-200 h-screen flex justify-center items-center">
      <div className="w-4/5 md:w-1/2 lg:w-2/5 bg-gray-100 rounded-3xl p-4 pt-6 pb-6 flex flex-col items-center shadow-2xl">
        <h1 className="text-black font-bold text-3xl font-sans mb-6">Create Account</h1>
        <p className="text-gray-500 font-sans text-center w-5/6 text-[18px] mb-4">Create a new account to get started and enjoy seamless access to out features.</p>
        <form className="min-w-11/12" onSubmit={handleSignup}>
          <div className="w-4/5 h-14 flex gap-5 items-center m-auto bg-white pl-4 rounded-2xl mb-4">
            <IoMdPerson size={28} />
            <input type="text" onChange={(e) => setName(e.target.value)} placeholder="Name" className="flex-1 h-full font-sans font-medium outline-none" required />
          </div>
          ...
          <div className="w-4/5 h-12 flex justify-center gap-5 items-center m-auto bg-green-400 text-white rounded-2xl mb-4">
            <button type="submit" className="h-full w-full font-medium font-sans cursor-pointer">Create Account</button>
          </div>

        </form>
        <p className="font-medium text-gray-500">Already have an account? <Link to="/login"><span className="text-green-600">Sign In here</span></Link></p>
      </div>
    </div>
  )
}
